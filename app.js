...
const closeTourPanel=()=>{currMuni=null;renderMap();tog(iAnc,'open',false);setTimeout(()=>{if(iTit)iTit.innerText='ZONA';if(iStat)iStat.innerHTML=`<div class="w-2 h-2 rounded-full bg-gray-600"></div><span class="text-gray-400 text-[10px] font-bold tracking-widest uppercase">Selecciona zona</span>`;if(iEvs)iEvs.innerHTML='';},300)};
const closeSetupPanel=()=>{$$('.comp-polygon').forEach(h=>h.classList.remove('hotspot-active','animate-neon'));tog(sAnc,'open',false);setTimeout(()=>{if(sTit)sTit.innerText='';if(sDesc)sDesc.innerText='';if(sStat)sStat.innerHTML=`<div class="w-2 h-2 rounded-full bg-gray-600"></div><span class="text-gray-400 text-[10px] font-bold uppercase">Hardware</span>`;},300)};
...
onValue(ref(db,'/'),s=>{
  const val = s.val() || {};
  dbData = { municipalities: val.municipalities || {}, components: val.components || {}, mashups: val.mashups || {} };
  try{renderMap()}catch(e){console.error(e)}
  try{renderComp()}catch(e){console.error(e)}
  try{renderMash()}catch(e){console.error(e)}
  try{updateDds()}catch(e){console.error(e)}
...
const uTrans=()=>{if(tLay)tLay.style.transform=`translate3d(${pX}px,${pY}px,0) scale(${sc})`;uLbls();reqFr=null};
...
if(vp) {
  on(vp,'mousedown',e=>{if(e.button!==0)return;stPan(e.clientX,e.clientY)});
...
fetch('salamanca.svg').then(r=>{if(!r.ok)throw new Error();return r.text()}).then(h=>{
  if(!tLay) return;
  tLay.innerHTML=h;
  const s=tLay.querySelector('svg');
  if(!s) { tLay.innerHTML = '<div class="text-djCyan text-center p-10 font-bold">Sin SVG válido.</div>'; return; }
  const g=crSvg('g');
  ...
  renderMap();
}).catch(() => { if(tLay) tLay.innerHTML = `<div class="text-red-500 font-bold text-center p-4">Sube 'salamanca.svg'.</div>`; });
...
const selMuni=id=>{currMuni=id;renderMap();const m=dbData.municipalities[id]||{},n=/^zona/i.test(m.name||id)?id.replace(/_/g,' '):(m.name||id.replace(/_/g,' '));if(iTit)iTit.innerText=n;if($('a-muni-id'))$('a-muni-id').value=id;if($('a-muni-name'))$('a-muni-name').value=n;clrEv();popEv(id);tog(iAnc,'open',true);if(m.status==='unlocked'){if($('a-muni-status'))$('a-muni-status').value='unlocked';if(iStat)iStat.innerHTML=`<div class="w-2 h-2 rounded-full bg-djCyan shadow-[0_0_8px_#00e5ff]"></div><span class="text-djCyan text-[10px] font-bold uppercase tracking-widest">Confirmadas</span>`;let evHtml='';if(m.events){Object.entries(m.events).forEach(([eid,ev])=>{const hasMedia=ev.media||ev.embed;evHtml+=`<div class="bg-black/60 backdrop-blur-md border border-white/10 rounded-2xl p-4 relative overflow-hidden"><div class="absolute top-0 left-0 w-1.5 h-full bg-djCyan shadow-[0_0_10px_#00e5ff]"></div><div class="flex justify-between items-start gap-2 relative z-10"><div><h4 class="text-sm font-display font-black text-white leading-tight">${ev.place}</h4><p class="text-[10px] text-djCyan font-bold uppercase tracking-wider mt-1">${ev.date}</p></div>${hasMedia?`<button onclick="document.getElementById('media-${eid}').classList.toggle('hidden');" class="bg-white/5 hover:bg-djCyan hover:text-black text-white w-8 h-8 rounded-xl border border-white/10 transition-colors flex items-center justify-center shrink-0 active:scale-95"><i class="fa-solid fa-chevron-down text-[10px]"></i></button>`:''}</div>${ev.details?`<p class="text-[11px] text-gray-300 mt-2 leading-relaxed">${ev.details}</p>`:''}${hasMedia?`<div id="media-${eid}" class="hidden mt-3 pt-3 border-t border-white/10 flex flex-col gap-2">${ev.media?ev.media.split(',').map(l=>(l=l.trim())?`<a href="${l}" target="_blank" class="w-full text-center py-2 bg-white/5 hover:bg-white/15 text-white border border-white/10 text-[10px] font-bold tracking-widest uppercase rounded-xl transition-all active:scale-95"><i class="fa-solid fa-ticket mr-2"></i>Entradas</a>`:'').join(''):''}${ev.embed?`<div class="w-full rounded-xl overflow-hidden shadow-xl border border-white/10 opacity-90 hover:opacity-100 transition-opacity text-[11px]">${ev.embed}</div>`:''}</div>`:''}</div>`})}else evHtml=`<p class="text-gray-500 text-sm text-center py-6">Próximamente.</p>`;if(iEvs)iEvs.innerHTML=evHtml}else{if($('a-muni-status'))$('a-muni-status').value='locked';if(iStat)iStat.innerHTML=`<div class="w-2 h-2 rounded-full bg-gray-600"></div><span class="text-gray-400 text-[10px] font-bold uppercase tracking-widest">Libre</span>`;if(iEvs)iEvs.innerHTML=`<div class="flex flex-col items-center justify-center py-8 text-center"><i class="fa-solid fa-envelope-open-text text-3xl text-gray-600 mb-3"></i><p class="text-gray-400 text-xs">Sin fechas confirmadas aquí.</p></div>`}if(iEvs)iEvs.innerHTML+=`<a href="https://instagram.com/djvaquero__" target="_blank" class="mt-auto block bg-gradient-to-r from-djCyan to-blue-600 text-black w-full py-3 rounded-xl text-center text-xs font-black uppercase tracking-wider hover:shadow-[0_0_20px_rgba(0,229,255,0.4)] active:scale-95 transition-all">Contratar</a>`};
if($('search-muni')) $('search-muni').oninput=e=>{const v=e.target.value.toLowerCase().trim(),rL=$('search-results');rL.innerHTML='';if(v.length<2)return tog(rL,'hidden',true);const grp={};Object.entries(dbData.municipalities||{}).forEach(([id,d])=>{if(d?.name&&!/^zona/i.test(d.name)&&d.name.toLowerCase().includes(v))grp[d.name]=grp[d.name]||id});const m=Object.entries(grp).slice(0,6);tog(rL,'hidden',false);if(m.length)m.forEach(([n,id])=>{const li=document.createElement('li');li.className="px-4 py-3 hover:bg-white/10 text-xs cursor-pointer border-b border-white/5 font-medium transition-colors text-gray-200";li.innerHTML=`<i class="fa-solid fa-location-dot text-djCyan mr-2 opacity-80"></i>${n}`;li.onclick=()=>{selMuni(id);$('search-muni').value='';tog(rL,'hidden',true)};rL.appendChild(li)});else rL.innerHTML=`<li class="px-4 py-3 text-gray-500 text-xs italic">Sin resultados</li>`};
on(document,'click',e=>{if($('search-muni')&&!$('search-muni').contains(e.target)&&$('search-results'))tog($('search-results'),'hidden',true)});

const renderMash=()=>{const g=$('mashups-grid');if(!g)return;const e=Object.entries(dbData.mashups||{}).sort();g.innerHTML=e.length?e.map(([_,d])=>`<div class="bg-black/60 backdrop-blur-md border border-white/10 p-5 rounded-2xl flex flex-col justify-between group overflow-hidden gap-3 relative transition-all hover:bg-black/80 hover:scale-[1.02] shadow-xl"><div class="absolute -right-4 -top-4 w-20 h-20 bg-djYellow/10 rounded-full blur-2xl group-hover:bg-djYellow/20 transition-colors"></div><div class="relative z-10"><h4 class="font-display font-black text-sm text-white leading-tight mb-2"><i class="fa-solid fa-music text-djYellow mr-2"></i>${d.title}</h4></div><div class="relative z-10 space-y-3">${d.scUrl?`<div class="w-full h-20 rounded-xl overflow-hidden border border-white/10 shadow-inner"><iframe width="100%" height="100%" scrolling="no" frameborder="no" src="https://w.soundcloud.com/player/?url=${encodeURIComponent(d.scUrl)}&color=%23ffd600&auto_play=false&hide_related=true&show_comments=false&show_teaser=false&visual=true"></iframe></div>`:''}${d.dl?`<a href="${d.dl}" target="_blank" class="w-full bg-djYellow/10 hover:bg-djYellow text-djYellow hover:text-black border border-djYellow/30 py-2.5 rounded-xl text-center text-[10px] font-black uppercase tracking-wider block transition-all hover:shadow-[0_0_15px_rgba(255,214,0,0.4)] active:scale-95"><i class="fa-solid fa-download mr-2"></i>Descargar</a>`:''}</div></div>`).join(''):'<p class="text-gray-500 text-sm p-4 col-span-full text-center">Aún no hay tracks disponibles.</p>'};

window.selComp=id=>{$$('.comp-polygon').forEach(h=>h.classList.remove('hotspot-active'));$$(`.comp-id-${id}`).forEach(h=>h.classList.add('hotspot-active'));const c=dbData.components[id];if(!c)return;tog(sAnc,'open',true);if(sStat)sStat.innerHTML=`<div class="w-2 h-2 rounded-full bg-djMagenta shadow-[0_0_10px_#ff00ea]"></div><span class="text-djMagenta text-[10px] font-bold uppercase tracking-widest">Activo</span>`;if(sTit)sTit.innerText=c.title;if(sDesc)sDesc.innerText=c.desc};
...
const renderAdminPts=()=>{
  ...
  figsUl.innerHTML=cFigs.map((f,i)=>{
    ...
    return `<li class="flex justify-between items-center bg-black/40 px-3 py-2 rounded-xl border border-white/10 text-[10px] transition-colors hover:border-djMagenta/50"><span class="text-gray-300"><i class="fa-solid fa-draw-polygon text-djMagenta/50 mr-2"></i>Polígono ${i+1} <span class="text-gray-500">(${f.length} ptos)</span></span><div class="flex gap-2"><button class="text-djCyan hover:text-white transition-colors p-1" onclick="window.editFig(${i})" title="Editar"><i class="fa-solid fa-pen"></i></button><button class="text-red-500 hover:text-red-400 transition-colors p-1" onclick="window.delFig(${i})" title="Eliminar"><i class="fa-solid fa-trash"></i></button></div></li>`;
  }).join('');
  ...
  cFigs.forEach((f,i)=>{
    ...
    p.setAttribute("stroke","rgba(255,0,234,0.5)");
  ...
  if(isDrawing){
    if(cPts.length>0){
      if(cPts.length>2){
        const p=crSvg("polygon");p.setAttribute("points",cPts.map(pt=>`${pt.x},${pt.y}`).join(' '));p.setAttribute("fill","rgba(0,229,255,0.1)");p.setAttribute("stroke","#00e5ff");p.setAttribute("stroke-width","2");p.setAttribute("stroke-dasharray","4");ov.appendChild(p);cPts.forEach((_,ni)=>crDot(cPts,ni,p,dc));
      } else {
        const p=crSvg("polyline");p.setAttribute("points",cPts.map(pt=>`${pt.x},${pt.y}`).join(' '));p.setAttribute("fill","none");p.setAttribute("stroke","#00e5ff");p.setAttribute("stroke-width","2");p.setAttribute("stroke-dasharray","4");ov.appendChild(p);cPts.forEach((_,ni)=>crDot(cPts,ni,p,dc));
      }
    }
    ptsUl.innerHTML=cPts.length?cPts.map((pt,i)=>`<li class="flex justify-between items-center bg-black/40 px-2 py-1.5 rounded-lg border border-white/5"><span class="font-mono text-[9px] text-gray-400"><span class="text-djCyan font-bold mr-1.5">${i+1}.</span> X: ${pt.x}% | Y: ${pt.y}%</span><button class="text-red-500/70 hover:text-red-500 transition-colors px-1" onclick="window.delNode(${i})"><i class="fa-solid fa-times"></i></button></li>`).join(''):'<li class="text-gray-500 text-center py-3 text-[10px] italic border border-dashed border-white/10 rounded-xl"><i class="fa-solid fa-hand-pointer block mb-2 text-sm"></i>Clic en la imagen para añadir nodos</li>';
...
window.edEv=(m,e)=>{const v=(dbData.municipalities[m]||{}).events[e]||{};if($('a-ev-id'))$('a-ev-id').value=e;if($('a-ev-p'))$('a-ev-p').value=v.place||'';if($('a-ev-d'))$('a-ev-d').value=v.date||'';if($('a-ev-desc'))$('a-ev-desc').value=v.details||'';if($('a-ev-m'))$('a-ev-m').value=v.media||'';if($('a-ev-embed'))$('a-ev-embed').value=v.embed||'';tog($('btn-cl-ev'),'hidden',false);if($('btn-sv-ev'))$('btn-sv-ev').innerText='OK';if($('a-ev-t'))$('a-ev-t').innerHTML='<i class="fa-solid fa-pen mr-2"></i> Editando'};
const popEv=id=>{const m=dbData.municipalities[id]||{},l=$('a-ev-list');if(!l)return;l.innerHTML='';if(m.events)Object.entries(m.events).forEach(([eid,ev])=>l.innerHTML+=`<div class="flex justify-between items-center bg-black/60 px-3 py-2 rounded-lg text-[11px] border border-white/5"><span class="truncate pr-2 font-bold text-gray-200">${ev.place}</span><div class="flex gap-3 shrink-0"><button class="text-djCyan hover:text-white transition-colors" onclick="window.edEv('${id}','${eid}')"><i class="fa-solid fa-pen"></i></button><button class="text-red-500 hover:text-red-400 transition-colors" onclick="window.delEv('${id}','${eid}')"><i class="fa-solid fa-trash"></i></button></div></div>`)};
const clrEv=()=>{['a-ev-id','a-ev-p','a-ev-d','a-ev-desc','a-ev-m','a-ev-embed'].forEach(i=>{if($(i))$(i).value=''});if($('a-ev-t'))$('a-ev-t').innerHTML='<i class="fa-solid fa-plus mr-2"></i> Fecha';if($('btn-sv-ev'))$('btn-sv-ev').innerText='Guardar';tog($('btn-cl-ev'),'hidden',true)};
...
if($('btn-sv-ms')) $('btn-sv-ms').onclick=async()=>{const t=$('a-mash-t')?.value;if(!t)return;const b=$('btn-sv-ms');b.disabled=true;b.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>';try{await set(ref(db,`mashups/${$('a-mash-sel').value==='NEW'?`mash_${Date.now()}`:$('a-mash-sel').value}`),{title:t,dl:$('a-mash-dl')?.value,scUrl:$('a-mash-sc')?.value});b.innerText='¡OK!';setTimeout(()=>{b.innerText='Publicar';b.disabled=false},1500)}catch(e){b.disabled=false;b.innerText='Error'}};
...
if($('admin-header')) on($('admin-header'),'pointerdown',e=>{if(e.target.closest('button'))return;drA=true;const p=$('admin-panel'),r=p.getBoundingClientRect();p.style.transform='none';p.style.left=r.left+'px';p.style.top=r.top+'px';dX=e.clientX-r.left;dY=e.clientY-r.top;});
on(window,'pointermove',e=>{if(drA&&$('admin-panel')){$('admin-panel').style.left=`${e.clientX-dX}px`;$('admin-panel').style.top=`${e.clientY-dY}px`}});
on(window,'pointerup',()=>drA=false);

['tour','setup','mashups'].forEach(t=>{if($(`tab-${t}`))$(`tab-${t}`).onclick=()=>{actTab=t;const c={tour:'djCyan',setup:'djMagenta',mashups:'djYellow'}[t];['tour','setup','mashups'].forEach(o=>{if($(`tab-${o}`))$(`tab-${o}`).className=o===t?`flex-1 py-3.5 text-${c} border-b-2 border-${c} bg-white/[0.03] transition-colors`:'flex-1 py-3.5 text-gray-500 border-b-2 border-transparent hover:text-white transition-colors';tog($(`sec-a-${o}`),'hidden',o!==t)});toggleAdminMode(t==='setup')}});
