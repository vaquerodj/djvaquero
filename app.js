  ...
  if(isDrawing){
    if(cPts.length>0){
      if(cPts.length>2){
        ...
        p.setAttribute("fill","rgba(0,229,255,0.1)");
        p.setAttribute("stroke","#00e5ff");
        p.setAttribute("stroke-width","1.5");
        p.setAttribute("stroke-dasharray","4");
        ov.appendChild(p);
        cPts.forEach((_,ni)=>crDot(cPts,ni,p,dc));
      } else {
        const p=crSvg("polyline");
        p.setAttribute("points",cPts.map(pt=>`${pt.x},${pt.y}`).join(' '));
...
$('btn-add-fig').onclick=()=>{isDrawing=true; cPts=[]; editFigIdx=-1; renderAdminPts()};
$('btn-cancel-fig').onclick=()=>{isDrawing=false; cPts=[]; editFigIdx=-1; renderAdminPts()};
$('btn-close-fig').onclick=()=>{
  if(cPts.length<3) return alert("Añade al menos 3 nodos (puntos) para poder cerrar la figura.");
  if(editFigIdx>=0) cFigs[editFigIdx]=[...cPts]; else cFigs.push([...cPts]);
  isDrawing=false; cPts=[]; editFigIdx=-1; renderAdminPts();
};

$('setup-wrapper').onclick=e=>{if(actTab==='setup'&&!$('admin-panel').classList.contains('hidden')&&isDrawing&&!e.target.classList.contains('admin-drag-dot')){const r=$('setup-wrapper').getBoundingClientRect();cPts.push({x:parseFloat(Math.max(0,Math.min(100,((e.clientX-r.left)/r.width)*100)).toFixed(1)),y:parseFloat(Math.max(0,Math.min(100,((e.clientY-r.top)/r.height)*100)).toFixed(1))});renderAdminPts()}};

$('btn-sv-cp').onclick=()=>{if(!$('a-comp-t').value)return;const aF=[...cFigs];if(isDrawing&&cPts.length>2)aF.push([...cPts]);set(ref(db,`components/${$('a-comp-sel').value==='NEW'?`comp_${Date.now()}`:$('a-comp-sel').value}`),{title:$('a-comp-t').value,desc:$('a-comp-d').value,figures:aF});isDrawing=false;cPts=[];editFigIdx=-1;renderAdminPts()};
...
$('a-comp-sel').onchange=e=>{const v=e.target.value;if(v==='NEW'){$('a-comp-t').value='';$('a-comp-d').value='';cFigs=[];cPts=[];isDrawing=false;editFigIdx=-1;tog($('btn-dl-cp'),'hidden',true)}else{const d=dbData.components[v];$('a-comp-t').value=d.title;$('a-comp-d').value=d.desc;cFigs=d.figures?d.figures.map(f=>[...f]):[];cPts=[];isDrawing=false;editFigIdx=-1;tog($('btn-dl-cp'),'hidden',false)}renderAdminPts()};
